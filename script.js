...
if (getCurrentPage() === 'questions.html') {
    currentQuestion = 0;
    score = 0;
    shuffleQuestions();
    showQuestion();
...
function startQuiz() {
    localStorage.setItem('score', 0),(localStorage.setItem('total', 0)),(localStorage.setItem('max', questions.length));
    localStorage.setItem('total', 0);
    localStorage.setItem('max', questions.length);
    goToScreen('questions.html');
    showQuestion();
    startCountdown();
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (page === 'gameover.html') {
        const scoreText = document.getElementById('score-text');
        const score = localStorage.getItem('score') || 0;
        const total = localStorage.getItem('total') || 0;
        const max = localStorage.getItem('max') || 0;
        if (scoreText) {
            scoreText.textContent = `Você acertou ${score} e respondeu ${total} de ${max} perguntas!`;
        }
    }
});
...
function gameOver() {
    ...
    localStorage.setItem('total', currentQuestion);
    localStorage.setItem('max', questions.length);
    goToScreen('gameover.html');
